<?php
    ...
    $commentuserid=$x['user_id'];
    ///RES-USER-ID
    $userid=$_GET['userid'];
    $result = mysqli_query($con, "SELECT * FROM  user WHERE user_id='$userid'");
    $x = mysqli_fetch_array($result);
    $per_auth = mysqli_query($con, "SELECT * FROM  personal_auth WHERE user_id='$userid' and status='1'");
    $job_auth = mysqli_query($con, "SELECT * FROM  job_auth WHERE user_id='$userid' and status='1'");
    ///AVERAGE
    $average = mysqli_query($con, "SELECT Avg(rating) AS Average FROM feedback where res_user_id='$userid' and status='1';");
    $p = mysqli_fetch_array($average);
    ...
    $per_auth_check=false;
    $job_auth_check=false;

    if(mysqli_num_rows($per_auth)>0 ){
      $per_auth_check=true;


  }
  if(mysqli_num_rows($job_auth)>0 ){
    $job_auth_check=true;
  ...
  if(isset($_POST['comment_submit'])){
    ...
    $feedbackInsert = mysqli_query($con, "INSERT INTO feedback (comment_user_id,res_user_id,rating,comment,date,status) VALUES ($commentuserid ,$userid,'$rating' , '$comment' ,CURRENT_DATE ,'0')" );
...
<body>
    ...
    <div class="main-div">
        ...
        <svg class='svg-icon' viewBox='0 0 20 20'>
            ...
            <span style="margin-right:381px;"><?php echo'  '?>امتیاز:<?php echo '  '.intval($p['Average'])?> از 5</span>
        </span>
            <br>
            <span><?php echo $x['firstname'].'  '.$x['lastname']; ?>  </span>
            <span style="margin-right: 350px;"> احراز هویت شخصی: <?php if($per_auth_check){echo "دارد";}else{echo "ندارد";}?></span>
            <br>
            <span>شماره تلفن:<?php echo '  '.$x['phone_num']?>  </span>  
            <span style="margin-right: 256px;"> احراز هویت شغلی: <?php if($job_auth_check){echo "دارد";}else{echo "ندارد";}?></span>
         
             <br>

            <span>ایمیل :<?php echo '  '.$x['email']?>  </span>
        ...
        <div  class="main-info yourfeedback" >
          <form method="post">
          <label>امتیاز خود را وارد کنید:</label>
              ...
              <br>
              <br>
                  <p class='text_user'>  نظر شما  : </p>
                            <textarea name='comment' rows='5' cols='50'></textarea>
                            <br>
                            <input style="font-family:iranyekanwebmedium  ; padding: 5px; margin-right:170px" type='submit' name='comment_submit'   value='ثبت نظر '>
          </form>
