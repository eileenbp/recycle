<?php
    ...
    $result2 = mysqli_query($con, "SELECT * FROM  auction WHERE auction_id=$auction_id");
    $result4 = mysqli_query($con, "SELECT * FROM  bid WHERE auction_id=$auction_id");

    $y = mysqli_fetch_array($result2);
    $w_unit=$y['weigth_unit'];
    if(isset($_POST['submit']))
    {
        ...
        $check2=mysqli_query($con, "INSERT INTO bid (user_id, auction_id , status,price,date,weigth,description,weigth_unit) VALUES ($userid,$auction_id,'0',$price,CURRENT_DATE,$weigth,'$description','$w_unit')");
        $_SESSION['status'] = "ثبت پیشنهاد شما با موفقیت صورت گرفت.";
...
<body>
    ...
    <div class="main-div">
        <div class=main-info>
        <span class="right-span"><img class="imagee" src="./imgs/upload/<?php echo $y['a_image'] ?>" alt="" ></span>
        <?php $userid=$y['user_id'];
        $resultx = mysqli_query($con, "SELECT username FROM  user WHERE user_id=$userid");
        $username = mysqli_fetch_array($resultx);?>
        <span><a class="txthover" href=<?php echo"./feedbacksubmit.php?userid=$userid" ?>><h5>برگزارکننده مزایده :<?php echo '  '.$username['username'] ?></h5></a></span>
        <span><h5>عنوان مزایده:<?php echo '  '.$y['title'] ?></h5></span>
        <?php 
        if($y['weigth_unit']=='0')
            $weigth_unit='تن';
        elseif($y['weigth_unit']=='1')
            $weigth_unit='کیلوگرم';
        else
            $weigth_unit='عدد';

        $residue_id=$y['residue_id'];
        $result3 = mysqli_query($con, "SELECT name FROM  residue WHERE residue_id=$residue_id");
        $z = mysqli_fetch_array($result3);    


               ?>
        <span id='weight' style="display:none;"><?php echo $y['weigth'];?></span>
        <span><h5>وزن:<?php echo '  '.$y['weigth'].'  '.$weigth_unit;?></h5></span>
        <span><h5>دسته آگهی:<?php echo '  '.$z['name'];?></h5></span>
        <span><h5> تاریخ شروع:<?php echo '  '.$y['start_date']; ?></h5></span>
        <span ><h5 style="">تاریخ پایان:<?php echo '  '.$y['end_date']; ?></h5></span>
        <span><h5>توضیحات:<?php echo '  '.$y['description']; ?></h5></span>
             <input id='btn_user_auth2' class='bid_open' type='submit' name='submit'  value='مشاهده همه پیشنهادات'>
        ...
        <?php
            ...
            <?php }?>
        <form method='post' name='bid-form' enctype='multipart/form-data'>
         
            
            <div class='div_user'>
                <p class='text_user'> وزن: </p>
                        <input class='input_user' name='w' id='w1' placeholder='وزن تقریبی' type='text' value=>
            ...
            <div class='div_user'>
                ...
                <label for='weight'>واحد:<?php echo '  '.$weigth_unit;?></label>
        ...
        <div class='div_user'>
            ...
            <div class='div_user'>
             ...
             <input id='btn_user_auth2' type='submit' name='submit'  value='ارسال پیشنهاد شما'>
